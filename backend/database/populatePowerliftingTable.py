...
def init_pl():
    ...
    print(
        'Completed download of open powerlifting data in {} seconds'.format(
            round(time.time() - startDownloadTime, 2)))
    ...
    try:
        ...
        with pd.read_csv('./openpowerlifting.csv', chunksize=50000, usecols=requiredColumns, dtype=dataTypes, parse_dates=dateColumns) as reader:
            for chunk in reader:
                chunk.columns = [colName.lower()
                                 for colName in requiredColumns]
                chunk.to_sql(
                    'powerlifting',
                    engine,
                    if_exists='append',
